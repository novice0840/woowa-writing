...
사람들과 팀 프로젝트를 하게 되면 `형상 관리 툴`을 필수적으로 사용하게 되고 그 중 가장 대중적인 툴이 `Git`라는 프로그램이다. 이 `Git`을 사용해 사람들과 협업을 할려면 원격에 저장소를 두고 소스 코드를 공유해야 하는데 이 때 가장 대표적으로 사용되는 것이 `Github`이다.
